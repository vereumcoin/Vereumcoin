...
#if defined(NDEBUG)
# error "Vereum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a2bb5c05f4be77e66544c187a51ca5093e98895ba5f81dd315bcafee9e272ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vereum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vereum Signed Message:\n";
...
{
    // Vereum: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vereum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vereum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1523457635, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Vereum VMC22 technology";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c4ecb3b65e5c55744cfdcfd83c61787cd8376fbdb7ecf1948972c21fbc9b5fa3e79f3925ea75856117ab22f7b91570ca3d9bd2eb0d18271033e03badc078528b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523457635;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2619259;
        ...
        assert(block.hashMerkleRoot == uint256("0xbd1d2abe6a6f14e4e42fcd779b749e4b5e88fd84f01fe34dc37948dae8aad476"));
...
unsigned char pchMessageStart[4] = { 0xf5, 0xe8, 0xa0, 0x61 }; // Vereum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VereumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VereumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VereumMiner(CWallet *pwallet)
{
    printf("VereumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vereum-miner");
    ...
    try { loop {
        ...
        printf("Running VereumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VereumMiner, pwallet));
}
